...
public class ErrorsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Find all GUI units
        errorsView = findViewById(R.id.graphView_errors);
        ArrayList<Double> error_values = new ArrayList<>();
        ...
        try /* to get all input */ {
            ...
            intent = new Intent(ErrorsActivity.this, DefineActivity.class);
        ...
        // Then, plot all error-lines
        if(approximated_euler_values.size() > 0){
            error_values.clear();
            error_values = methods.get_difference(approximated_euler_values, exact_values);
            series_euler = create_line_series(error_values, Color.BLUE, "Error of Euler method");
            errorsView.addSeries(series_euler);
        }
        if(approximated_improved_euler_values.size() > 0){
            error_values.clear();
            error_values = methods.get_difference(approximated_improved_euler_values, exact_values);
            series_euler_plus = create_line_series(error_values, Color.GREEN, "Error of improved Euler method");
            errorsView.addSeries(series_euler_plus);
        }
        if(approximated_runge_kutta_values.size() > 0){
            error_values.clear();
            error_values = methods.get_difference(approximated_runge_kutta_values, exact_values);
            series_runge_kutta = create_line_series(error_values, Color.RED, "Error of Runge-Kutta method");
            errorsView.addSeries(series_runge_kutta);
        }
        if(exact_values.size() > 0){
            error_values.clear();
            error_values = methods.get_difference(exact_values, exact_values);
            series_exact = create_line_series(error_values, Color.YELLOW, "Exact solution error == 0");
            errorsView.addSeries(series_exact);
        }
